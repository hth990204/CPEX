...
"""默认切片深度为1：LEVEL_SLICE=1 可修改1-5 2024年1月10日18:06:22"""
...
def subprocess_cmd_joern(command: str):
    ...
    """
    try:
        output = check_output(command, stderr=STDOUT, timeout=None, shell=True)
...
def not_java_file(file_name):
    ext = file_name.rsplit('.', 1)
    if len(ext) < 2 or (len(ext) > 1 and ext[1] not in ['java']):
        return True
    return False